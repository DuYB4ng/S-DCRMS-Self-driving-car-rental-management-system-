...
const CarManagementPage = () => {
  ...
  const fetchCars = async () => {
    try {
      ...
      const mappedCars = response.data.map((c) => ({
        ...
        price: c.pricePerDay,
        state: c.isAvailable,
        status: c.isAvailable ? "Sẵn sàng" : "Không sẵn sàng",
        image:
  ...
  const handleUpdateCar = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:8001/api/car/${selectedCar.id}`,
        updateDTO
      );
  ...
  const handleDeleteCar = async (carId) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa xe này?")) {
      try {
        await axios.delete(`http://localhost:8001/api/car/${carId}`);
        fetchCars();
      } catch (error) {
  ...
  const stats = [
    ...
    {
      label: "Sẵn sàng",
      value: carsList.filter((c) => c.state).length,
      color: "text-success",
    },
    {
      label: "Đang thuê",
                ...
                <div>
                  <div className="bg-gradient-to-br from-[#2E7D9A] to-[#1a4d5e] text-white rounded-xl p-6 mb-6">
                    ...
                    <p className="text-4xl font-bold">
                      {(selectedCar?.price || 0).toLocaleString()}đ
                    </p>
              ...
              <div className="bg-secondary rounded-xl p-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center p-4 bg-white rounded-lg">
                    <p className="text-2xl font-bold text-primary">
                      {(
                        (selectedCar?.price || 0) * (selectedCar?.trips || 0)
                      ).toLocaleString()}
                      đ đ
                    </p>
